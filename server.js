...
app.post('/api/qa', async (req, res) => {
  ...
  try {
    // 1. 药剂相关问题
    if (q.includes('药剂') || q.includes('加药') || q.includes('投加')) {
      const dosing = await new Promise((resolve, reject) => {
        db.all(`SELECT * FROM chemical_dosing ORDER BY id DESC LIMIT 1`, (err, rows) => {
          if (err) reject(err);
          else resolve(rows);
        });
      });
      
      if (q.includes('pac') || q.includes('聚合氯化铝')) {
        answer = `根据最新数据，PAC推荐投加量为 ${dosing.pac || '?'} kg/d。`;
      } else if (q.includes('碳源') || q.includes('甲醇') || q.includes('乙酸')) {
        answer = `根据最新数据，碳源推荐投加量为 ${dosing.carbon || '?'} kg/d。`;
      } else {
        answer = '药剂投加建议：请访问「智能加药」页面查看完整推荐。';
      }
    }
    // 2. 水质相关问题
    else if (q.includes('水质') || q.includes('cod') || q.includes('氨氮') || q.includes('总磷') || q.includes('总氮')) {
      const lab = await new Promise((resolve, reject) => {
        db.all(`SELECT * FROM daily_lab ORDER BY id DESC LIMIT 1`, (err, rows) => {
          if (err) reject(err);
          else resolve(rows);
        });
      });
...
- 出水总磷: ${l.outTp || '?'} mg/L`;
    ...
    // 3. 库存相关问题
    else if (q.includes('库存') || q.includes('剩余') || q.includes('还有')) {
      const inv = await new Promise((resolve, reject) => {
        db.all(`SELECT chemicalName, SUM(amount) as total FROM chemical_inventory GROUP BY chemicalName`, (err, rows) => {
          if (err) reject(err);
          else resolve(rows);
        });
      });
      ...
      if (inv && inv.length > 0) {
        answer = '当前药剂库存：
' + inv.map(x => `- ${x.chemicalName}: ${x.total} kg`).join('
');
      } else {
        answer = '暂无库存数据。';
      }
    ...
    // 4. 巡检相关问题
    else if (q.includes('巡检') || q.includes('do') || q.includes('溶解氧')) {
      const insp = await new Promise((resolve, reject) => {
        db.all(`SELECT * FROM do_inspection ORDER BY id DESC LIMIT 5`, (err, rows) => {
          if (err) reject(err);
          else resolve(rows);
        });
      });
      ...
      if (insp && insp.length > 0) {
        answer = `最近${insp.length}条巡检记录：
` + insp.map(x => `- ${x.time || x.date}: ${x.area} DO=${x.doValue}mg/L`).join('
');
      } else {
        answer = '暂无巡检数据。';
      }
